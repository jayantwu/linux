...
void echo_cli(int sock) {
    ...
    int stdineof = 0;
	//利用select处理标准输入I/O事件和网络read I/O事件 ，因为如果服务器关闭之后，会发送FIN报文，客户端会阻塞在fget标准输入，无法完成read，即无法处理FIN 
	while(1) 
	{
        if(stdineof == 0) 
            FD_SET(fd_stdin, &rset);  //将标准输入加入到读集合 
		FD_SET(sock, &rset);   //将sockfd加入到读集合 
		nready = select(maxfd + 1, &rset, NULL, NULL, NULL);  //如果有读事件发生，select函数返回发生的个数 
		if(nready == -1)
			ERR_EXIT("select");
		if(nready == 0)
			continue;
		if(FD_ISSET(sock, &rset)) //网络读事件发生 
		{
