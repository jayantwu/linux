...
int main() {
	...
	*/
	...
	client[0].events = POLLIN;//对可读事件感兴趣 ，表示关心可读事件 
	
	while(1) {
		nready = poll(client, maxi + 1, -1);    //int poll(struct pollfd *fds, nfds_t nfds, int timeout); 第一个参数是一个结构体数组 nfds应该时内部用来循环遍历的
		if(nready == -1) {
			if(errno == EINTR)//被信号中断
				continue;
			ERR_EXIT("poll");
		...
		if(nready == 0)//超时，现在timeout设置为NULL，故不可能发生
			continue; 
		//当监听套接字产生了可读事件时 
		if(client[0].revents & POLLIN) {  //类似于位图 ，用与 ，哪个事件发生时，就会更新client[i]结构体的revent成员的值 
			peerlen = sizeof(peeraddr);//必须设置一个初始值 
			conn = accept(listenfd, (struct sockaddr*)&peeraddr, &peerlen);  //peeraddr结构体作为出参 
			if(conn == -1)
				ERR_EXIT("accept"); 
			//保存到某个空闲位置 
			for(i = 0; i < 2048; ++i) {
			...
			printf("ip=%s port=%d\n", inet_ntoa(peeraddr.sin_addr), ntohs(peeraddr.sin_port));//打印链接过来的客户端的地址信息 
			
			client[i].events = POLLIN;   //关心这个 连接的可读事件 
		...
		for(i = 1; i <= maxi; ++i) {
			...
			if(client[i].events & POLLIN) {
				char recvbuf[1024] = {0};
				int ret = readline(conn, recvbuf, 1024);//这个时候readline不会被阻塞，因为已经有可读事件产生了 
				if(ret == -1)
					ERR_EXIT("readline");
				if(ret == 0) {
					printf("client close\n");
